...
BOT_TOKEN = "8695257197:AAGCKRxreBMjgU-pCu8cF5_S5sJYuOkSBDw"
...
# Group links (add more as you scale)
group_links = {
...
def find_related_groups(source, destination, bucket):
    ...
    return results[:3]  # ⚡ KEEP CONTROL (max 3)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Final step
        ...
        # 🎯 Exact match
        if match_count > 0:
            await update.message.reply_text(f"🎉 Found {match_count} people on same route!")
            ...
            if key in group_links:
                ...
                await update.message.reply_text(
                    "Group will be available soon for this route."
                )

        # 🔥 No match → show related
        else:
            ...
            if related:
                msg = "👉 You can join similar routes:\n\n"
                msg += "\n\n".join(related)

                await update.message.reply_text(msg)
            else:
                await update.message.reply_text(
                    "We’ll notify you once more people join your route."
                )
...
print("🚀 Bot running with controlled related routes...")
app.run_polling()